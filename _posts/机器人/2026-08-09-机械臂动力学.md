...
### 一个算得动的例子：平面 2R 臂的完整推导

杆长 $L_1, L_2$，质量简化为集中在杆末端的点质量 $m_1, m_2$（真实连杆用质心加惯量张量，推导同理只是更长）。下面把推导的每一步走完，记 $c_1 = \cos q_1$，$c_{12} = \cos(q_1{+}q_2)$，其余类推。

**第一步：位置与速度。**两个质点的位置由几何直接写出：

$$
\mathbf p_1 = \begin{bmatrix} L_1 c_1 \\ L_1 s_1 \end{bmatrix},
\qquad
\mathbf p_2 = \begin{bmatrix} L_1 c_1 + L_2 c_{12} \\ L_1 s_1 + L_2 s_{12} \end{bmatrix}
$$

对时间求导（链式法则，注意 $c_{12}$ 对时间的导数带因子 $\dot q_1 + \dot q_2$）：

$$
\mathbf v_1 = L_1 \dot q_1 \begin{bmatrix} -s_1 \\ c_1 \end{bmatrix},
\qquad
\mathbf v_2 = \begin{bmatrix} -L_1 s_1 \dot q_1 - L_2 s_{12} (\dot q_1 + \dot q_2) \\ L_1 c_1 \dot q_1 + L_2 c_{12} (\dot q_1 + \dot q_2) \end{bmatrix}
$$

**第二步：动能。**$\|\mathbf v_1\|^2 = L_1^2 \dot q_1^2$ 是显然的；$\|\mathbf v_2\|^2$ 展开后交叉项里出现 $s_1 s_{12} + c_1 c_{12}$，由余弦差角公式它恰好等于 $\cos\big((q_1{+}q_2) - q_1\big) = c_2$——**方程里所有 $\cos q_2$ 的源头就在这一步**：

$$
\|\mathbf v_2\|^2 = L_1^2 \dot q_1^2 + L_2^2 (\dot q_1 + \dot q_2)^2 + 2 L_1 L_2\, c_2\, \dot q_1 (\dot q_1 + \dot q_2)
$$

$$
T = \tfrac12 m_1 L_1^2 \dot q_1^2 + \tfrac12 m_2 \left[ L_1^2 \dot q_1^2 + L_2^2 (\dot q_1{+}\dot q_2)^2 + 2 L_1 L_2 c_2\, \dot q_1 (\dot q_1{+}\dot q_2) \right],
\qquad
V = m_1 g L_1 s_1 + m_2 g \left( L_1 s_1 + L_2 s_{12} \right)
$$

**第三步：对 $q_1$ 摇欧拉-拉格朗日。**先求动量项：

$$
\frac{\partial T}{\partial \dot q_1} = (m_1{+}m_2) L_1^2 \dot q_1 + m_2 L_2^2 (\dot q_1{+}\dot q_2) + m_2 L_1 L_2 c_2 (2\dot q_1 + \dot q_2)
$$

对时间求全导数时，$c_2$ 也是时间的函数（$\dot c_2 = -s_2 \dot q_2$），**科氏/离心项就是从这里漏出来的**：

$$
\frac{\mathrm d}{\mathrm d t}\frac{\partial T}{\partial \dot q_1}
= \underbrace{\left[(m_1{+}m_2)L_1^2 + m_2 L_2^2 + 2 m_2 L_1 L_2 c_2\right]\ddot q_1 + \left[m_2 L_2^2 + m_2 L_1 L_2 c_2\right]\ddot q_2}_{\text{将成为 } M_{11}\ddot q_1 + M_{12}\ddot q_2}
\;\underbrace{-\; m_2 L_1 L_2 s_2\, \dot q_2 (2\dot q_1 + \dot q_2)}_{\text{速度二次项}}
$$

$T$ 不含 $q_1$（$\partial T/\partial q_1 = 0$，这是关节 1 的平移对称性），$\partial V/\partial q_1 = (m_1{+}m_2) g L_1 c_1 + m_2 g L_2 c_{12}$。装配起来就是第一行方程。

**第四步：对 $q_2$ 摇一遍。**这次 $\partial T/\partial q_2 \ne 0$（$T$ 通过 $c_2$ 依赖 $q_2$）：

$$
\frac{\partial T}{\partial \dot q_2} = m_2 L_2^2 (\dot q_1{+}\dot q_2) + m_2 L_1 L_2 c_2 \dot q_1,
\qquad
\frac{\partial T}{\partial q_2} = -m_2 L_1 L_2 s_2\, \dot q_1 (\dot q_1 + \dot q_2)
$$

$$
\frac{\mathrm d}{\mathrm d t}\frac{\partial T}{\partial \dot q_2} - \frac{\partial T}{\partial q_2}
...
**第五步：按结构归位。**把两行按 $\ddot{\mathbf q}$、速度二次项、重力整理，得到
...
回看整个过程，三项的出身一目了然：$\mathbf M$ 是动能对 $\dot{\mathbf q}$ 二次型的系数；$\mathbf C\dot{\mathbf q}$ 全部来自"$\mathbf M$ 随位形变化"漏出的两处——$\frac{\mathrm d}{\mathrm dt}$ 作用在 $c_2$ 上（第三步）和 $\partial T/\partial q_2$（第四步）；$\mathbf G$ 就是 $\partial V/\partial \mathbf q$。任意自由度的机械臂都是同一套流程，只是手工展开在 $n \ge 3$ 时不再现实。
...
2. 取 Christoffel 符号约定的 $\mathbf C$ 时，$\dot{\mathbf M} - 2\mathbf C$ **反对称**。这条性质值得亲手验证一次。首先注意 $\mathbf C\dot{\mathbf q}$ 作为向量并不唯一确定矩阵 $\mathbf C$（同一个乘积可以拆成不同矩阵），Christoffel 约定取

   $$
   C_{ij} = \sum_k c_{ijk}\,\dot q_k, \qquad
   c_{ijk} = \frac12\left(\frac{\partial M_{ij}}{\partial q_k} + \frac{\partial M_{ik}}{\partial q_j} - \frac{\partial M_{kj}}{\partial q_i}\right)
   $$

   对 2R 臂，$\mathbf M$ 中唯一随位形变的量是 $c_2$，记 $h = m_2 L_1 L_2 \sin q_2$，则 $\partial M_{11}/\partial q_2 = -2h$、$\partial M_{12}/\partial q_2 = -h$、其余偏导为零。代入逐项算得

   $$
   \mathbf C = \begin{bmatrix} -h\dot q_2 & -h(\dot q_1 + \dot q_2) \\ h\dot q_1 & 0 \end{bmatrix},
   \qquad
   \dot{\mathbf M} = \begin{bmatrix} -2h\dot q_2 & -h\dot q_2 \\ -h\dot q_2 & 0 \end{bmatrix}
   $$

   （可以先验证 $\mathbf C\dot{\mathbf q}$ 确实还原出第五步的速度二次项。）于是

   $$
   \dot{\mathbf M} - 2\mathbf C = \begin{bmatrix} 0 & h(2\dot q_1 + \dot q_2) \\ -h(2\dot q_1 + \dot q_2) & 0 \end{bmatrix}
   $$

   ——严格反对称。它的物理含义是能量守恒：把动力学方程代入动能变化率，得 $\frac{\mathrm d}{\mathrm dt}\big(\tfrac12\dot{\mathbf q}^\top\mathbf M\dot{\mathbf q}\big) = \dot{\mathbf q}^\top(\boldsymbol\tau - \mathbf G) + \dot{\mathbf q}^\top\big(\tfrac12\dot{\mathbf M} - \mathbf C\big)\dot{\mathbf q}$，而反对称矩阵的二次型恒为零，末项消失——科氏/离心力**改变动量方向但不注入能量**。基于无源性（passivity）的控制器（PD + 重力补偿的全局稳定性证明）就建在这条性质上；
