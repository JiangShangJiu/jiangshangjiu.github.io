...
### 3.2 误差动力学是推出来的，不是拼出来的

$\mathbf F$ 的每一块都来自一条连续时间微分方程，值得完整推一遍。目标：**姿态误差 $\delta\boldsymbol\theta$ 自己怎么随时间演化**。

用旋转矩阵形式最干净。误差定义 $\mathbf R_{true} = \mathbf R_{nom}\,\delta\mathbf R$，小角度下 $\delta\mathbf R \approx \mathbf I + [\delta\boldsymbol\theta]_\times$。两条轨迹各自服从姿态运动学（角速度在体坐标系）：

$$
\dot{\mathbf R}_{true} = \mathbf R_{true} [\boldsymbol\omega_{true}]_\times,
\qquad
\dot{\mathbf R}_{nom} = \mathbf R_{nom} [\hat{\boldsymbol\omega}]_\times
$$

其中 $\hat{\boldsymbol\omega} = \boldsymbol\omega_m - \mathbf b_{nom}$ 是名义积分用的角速度，而真值 $\boldsymbol\omega_{true} = \boldsymbol\omega_m - \mathbf b_{true} - \mathbf n_g = \hat{\boldsymbol\omega} - \delta\mathbf b - \mathbf n_g$——**误差的源头就是这两者之差**。

对 $\mathbf R_{true} = \mathbf R_{nom}\delta\mathbf R$ 两边求导（乘积法则），代入两条运动学，并左乘 $\mathbf R_{nom}^\top$ 消去名义旋转：

$$
\delta\dot{\mathbf R} = \delta\mathbf R\, [\boldsymbol\omega_{true}]_\times - [\hat{\boldsymbol\omega}]_\times\, \delta\mathbf R
$$

代入一阶近似 $\delta\mathbf R \approx \mathbf I + [\delta\boldsymbol\theta]_\times$、$\delta\dot{\mathbf R} \approx [\delta\dot{\boldsymbol\theta}]_\times$，展开并丢掉二阶小量（$[\delta\boldsymbol\theta]_\times[\delta\mathbf b]_\times$ 之类）：

$$
[\delta\dot{\boldsymbol\theta}]_\times
= \underbrace{[\delta\boldsymbol\theta]_\times [\hat{\boldsymbol\omega}]_\times - [\hat{\boldsymbol\omega}]_\times [\delta\boldsymbol\theta]_\times}_{\text{对易子}} \;-\; [\delta\mathbf b + \mathbf n_g]_\times
$$

对易子用叉乘恒等式 $[\mathbf a]_\times[\mathbf b]_\times - [\mathbf b]_\times[\mathbf a]_\times = [\mathbf a \times \mathbf b]_\times$ 收拢成一项，去掉两边的 $[\cdot]_\times$：

$$
\boxed{\;\delta\dot{\boldsymbol\theta} = -\hat{\boldsymbol\omega} \times \delta\boldsymbol\theta - \delta\mathbf b - \mathbf n_g\;}
\qquad
\delta\dot{\mathbf b} = \mathbf n_{bg}
$$

三项各有含义：$-\hat{\boldsymbol\omega}\times\delta\boldsymbol\theta$ 说明机体旋转时姿态误差矢量会**跟着坐标系转**（误差表示在体坐标系里，载体转了、误差的分量表达就变）；$-\delta\mathbf b$ 是偏置误差按 1:1 的速率灌进姿态误差；$\mathbf n_g$ 是白噪声直接驱动。偏置自己按随机游走漂移。

对这组线性微分方程做 $\Delta t$ 的零阶保持离散化，就得到上面的 $\mathbf F$：左上块是 $\delta\boldsymbol\theta$ 齐次方程的状态转移矩阵 $\exp(-[\hat{\boldsymbol\omega}\Delta t]_\times) = \exp(-[\delta\boldsymbol\phi]_\times)$（这是个旋转矩阵，可用 Rodrigues 公式精确算），右上块 $-\mathbf I\Delta t$ 是 $-\delta\mathbf b$ 项的一阶积分，右下的单位块对应随机游走。**整个滤波器里唯一的近似就是"丢掉二阶小量"这一步**，而误差状态每个周期都被复位到零，二阶小量是真的小——这就是 ESKF 线性化质量优于 EKF 的推导层面依据。完整的含速度、位置的 15 维版本见 Solà 的讲义，套路与此完全相同。
