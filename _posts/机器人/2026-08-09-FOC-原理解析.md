...
**系数 $\tfrac23$ 从哪来？**把三相合成的电流矢量写成复数形式，三个绕组轴线方向分别是 $1, \mathbf a, \mathbf a^2$（$\mathbf a = e^{j 2\pi/3}$），合成矢量取

$$
\vec i_s = k\left(i_a + \mathbf a\, i_b + \mathbf a^2 i_c\right)
$$

代入对称三相电流 $i_a = I\cos\theta$、$i_b = I\cos(\theta - \tfrac{2\pi}{3})$、$i_c = I\cos(\theta + \tfrac{2\pi}{3})$，用 $\cos x = \tfrac12(e^{jx} + e^{-jx})$ 展开。正频分量 $e^{j\theta}$ 的系数是 $\tfrac{I}{2}(1 + \mathbf a\,\mathbf a^{-1}\cdot 1 + \mathbf a^2 \mathbf a^{-2}) = \tfrac{3I}{2}$，负频分量的系数是 $\tfrac{I}{2}(1 + \mathbf a\, \mathbf a^{-2}\cdots) = \tfrac{I}{2}(1 + \mathbf a^2 + \mathbf a^4) = 0$（三次单位根之和为零）。于是

$$
\vec i_s = k\,\frac{3I}{2}\, e^{j\theta}
$$

——三相对称电流合成一个**幅值恒定、匀速旋转**的矢量，取 $k = \tfrac23$ 恰好让矢量幅值等于相电流幅值 $I$，这就是等幅值约定；取 $k = \sqrt{2/3}$ 则保持功率不变（等功率约定），两种约定的转矩公式差一个系数，混用是新手常见错误。上面的矩阵就是 $k = \tfrac23$ 时取实部、虚部的结果。

星形接法且中性点不引出时 $i_a + i_b + i_c = 0$，代入可消去一相：
...
$$
\vec u_{\alpha\beta} = R_s\, \vec i_{\alpha\beta} + \frac{\mathrm d \vec\psi_{\alpha\beta}}{\mathrm d t}
$$

把任意矢量换到以 $\theta_e$ 旋转的 dq 系：$\vec x_{\alpha\beta} = \vec x_{dq}\, e^{j\theta_e}$。关键一步是对乘积求导——**旋转坐标系里求导要多出一项**：

$$
\frac{\mathrm d \vec x_{\alpha\beta}}{\mathrm d t}
= \frac{\mathrm d \vec x_{dq}}{\mathrm d t} e^{j\theta_e} + j\omega_e\, \vec x_{dq}\, e^{j\theta_e}
\quad\Longrightarrow\quad
\left.\frac{\mathrm d}{\mathrm d t}\right|_{\alpha\beta} = \left.\frac{\mathrm d}{\mathrm d t}\right|_{dq} + j\omega_e
$$

（这正是理论力学里"旋转系中的科氏项"，只是写成了复数。）代入电压方程并约去 $e^{j\theta_e}$：

$$
\vec u_{dq} = R_s\, \vec i_{dq} + \frac{\mathrm d \vec\psi_{dq}}{\mathrm d t} + j\omega_e\, \vec\psi_{dq}
$$

dq 系的好处在磁链表达式上兑现：d 轴对准转子磁链后，$\psi_d = L_d i_d + \psi_f$、$\psi_q = L_q i_q$（$\psi_f$ 是永磁体磁链，在 dq 系里是**常数**——这就是换坐标系的全部动机）。代入并把 $j\omega_e \vec\psi_{dq} = j\omega_e(\psi_d + j\psi_q) = -\omega_e \psi_q + j\omega_e \psi_d$ 按实部（d）、虚部（q）拆开：
...
三个"多出来"的项全部来自那一个 $j\omega_e$：

- $-\omega_e L_q i_q$ 与 $+\omega_e L_d i_d$：**交叉耦合项**，d、q 轴互相干扰——本质是旋转系里的运动电动势；
- $\omega_e \psi_f$：**反电动势**，随转速线性增大——永磁磁链在旋转系里"看似不动"，但它对静止绕组的相对运动实实在在地感应电压。
...
三相桥有 $2^3 = 8$ 种开关组合。先算清基本矢量的幅值：以状态 100（A 相上管通、B/C 下管通）为例，三相端电压为 $(\tfrac{2}{3}U_{dc}, -\tfrac13 U_{dc}, -\tfrac13 U_{dc})$（对中性点，由星形绕组分压得出），代入 Clarke 变换得 $\vec U_1 = \tfrac23 U_{dc} \angle 0°$。六个非零矢量幅值均为 $\tfrac23 U_{dc}$、彼此相隔 60°，把平面分成六个扇区；000 与 111 两个零矢量位于原点。

![SVPWM 六个基本电压矢量与参考矢量合成](/assets/img/foc/svpwm-sectors.svg)
_参考矢量落在扇区 I 时，用相邻的 $\vec U_1$、$\vec U_2$ 与零矢量按伏秒平衡合成_

逆变器一个周期内只能输出这 8 个离散矢量，怎么"输出"任意的 $\vec U_{ref}$？答案是时间平均——**伏秒平衡**：电流环的等效对象是电感，电感电流的增量由电压对时间的积分（伏秒积）决定，只要一个开关周期 $T_s$ 内的伏秒积相同，对电流的作用就等效：

$$
\vec U_{ref}\, T_s = \vec U_1 T_1 + \vec U_2 T_2 + \vec U_{0,7}\, T_0
$$

设 $\vec U_{ref}$ 幅值 $U$、幅角 $\theta$ 落在扇区 I。把矢量方程按实轴、虚轴投影成两个标量方程（$\vec U_1$ 沿 $0°$，$\vec U_2$ 沿 $60°$）：
...
第二式直接给出 $T_2$；代回第一式，用差角公式 $\tfrac{\sqrt3}{2}\cos\theta - \tfrac12\sin\theta = \sin(\tfrac{\pi}{3} - \theta)$ 整理出 $T_1$：

$$
T_1 = \frac{\sqrt3\, T_s\, U}{U_{dc}} \sin\!\left(\frac{\pi}{3} - \theta\right),
\qquad
...
两个作用时间恰好是 $\vec U_{ref}$ 到扇区两条边的"角度距离"的正弦——图中绿色、橙色两段分解即是。$T_0 \ge 0$ 是可行性条件，它等价于 $\vec U_{ref}$ 落在六边形内。其余扇区不用重新推导：把 $\theta$ 减去扇区起始角（$60°$ 的整数倍）即可复用同一组公式，这也是代码里"扇区判断 + 查表"结构的由来。
