...
协方差方程可以直接从误差的定义推出来：记估计误差 $\mathbf e_{k-1} = \mathbf x_{k-1} - \hat{\mathbf x}_{k-1}$，预测误差为 $\mathbf e_k^- = \mathbf A \mathbf e_{k-1} + \mathbf w_k$。误差与过程噪声不相关（$\mathbf w_k$ 是未来的噪声，不可能影响过去的估计），所以交叉项期望为零：

$$
\mathbf P_k^- = \mathbb E\left[\mathbf e_k^- (\mathbf e_k^-)^\top\right]
= \mathbf A\, \mathbb E[\mathbf e_{k-1}\mathbf e_{k-1}^\top]\, \mathbf A^\top + \mathbb E[\mathbf w_k \mathbf w_k^\top]
= \mathbf A \mathbf P_{k-1} \mathbf A^\top + \mathbf Q
$$
...
设 $\hat{\mathbf x}_k = \hat{\mathbf x}_k^- + \mathbf K \mathbf y_k$，$\mathbf K$ 待定。更新后误差为
...
= (\mathbf I - \mathbf K\mathbf H)\, \mathbf e_k^- - \mathbf K \mathbf v_k
$$

$\mathbf e_k^-$ 与 $\mathbf v_k$ 不相关，于是更新后协方差是 $\mathbf K$ 的二次函数（这一步称为 Joseph 形式，对**任意** $\mathbf K$ 成立）：

$$
\mathbf P_k(\mathbf K) = (\mathbf I - \mathbf K\mathbf H)\, \mathbf P_k^- (\mathbf I - \mathbf K\mathbf H)^\top + \mathbf K \mathbf R \mathbf K^\top
$$

"误差最小"取均方误差 $\mathbb E[\|\mathbf e_k\|^2] = \operatorname{tr} \mathbf P_k$。对 $\mathbf K$ 求导（用矩阵导数公式 $\tfrac{\partial}{\partial \mathbf K}\operatorname{tr}(\mathbf K \mathbf S \mathbf K^\top) = 2\mathbf K\mathbf S$ 与 $\tfrac{\partial}{\partial \mathbf K}\operatorname{tr}(\mathbf K \mathbf U^\top) = \mathbf U$）：

$$
\frac{\partial \operatorname{tr}\mathbf P_k}{\partial \mathbf K}
= -2\,(\mathbf P_k^-)\mathbf H^\top + 2\,\mathbf K\left(\mathbf H \mathbf P_k^- \mathbf H^\top + \mathbf R\right) = \mathbf 0
$$

括号里的矩阵正是**新息的协方差** $\mathbf S = \mathbf H \mathbf P_k^- \mathbf H^\top + \mathbf R$（新息 $\mathbf y_k = \mathbf H\mathbf e_k^- + \mathbf v_k$，两部分独立、协方差相加）。解出

$$
\mathbf K_k = \mathbf P_k^- \mathbf H^\top \mathbf S^{-1}
$$

——增益的结构是"状态与观测的互协方差 $\times$ 新息协方差的逆"，即**新息里每一分信息按它的可信度折算成状态修正**。把最优 $\mathbf K_k$ 代回 Joseph 形式，二次项与交叉项部分抵消，剩下

$$
\mathbf P_k = (\mathbf I - \mathbf K_k \mathbf H)\,\mathbf P_k^-
$$

注意这个简洁形式**只在 $\mathbf K$ 取最优值时成立**；数值实现中若增益被修改过（比如做了限幅），必须退回 Joseph 形式更新协方差，否则 $\mathbf P$ 会失去对称正定性。

同一组公式也可以从贝叶斯路线得到：把 $\overline{bel}$ 和似然两个高斯指数相加、对 $\mathbf x_k$ 配方，得到的后验均值与协方差和上面完全一致——**线性高斯情形下 MMSE 最优解与贝叶斯后验是同一个东西**，这就是正文说"不是近似，是最优"的依据。
