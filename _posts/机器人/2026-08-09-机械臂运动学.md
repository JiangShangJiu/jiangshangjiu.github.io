...
其中 $\mathbf z_{i-1}$、$\mathbf p_{i-1}$ 分别是第 $i$ 个关节轴方向和轴上一点的位置（正解的中间结果，顺手就有）。

**这个逐列公式为什么成立？**分两步说清。

第一步，为什么可以一列一列地看。速度映射 $\mathbf J\dot{\mathbf q} = \sum_i \mathbf J_i \dot q_i$ 对 $\dot{\mathbf q}$ 是线性的，所以末端速度是"每个关节单独运动产生的速度"的叠加。冻结除 $i$ 外的所有关节，此时从关节 $i$ 到末端的整段机构是一个**刚体**，末端的运动完全由关节 $i$ 的运动决定——这就把问题化成了单关节的刚体运动学。

第二步，单个转动关节产生什么速度。关节 $i$ 以 $\dot q_i$ 绕轴 $\mathbf z_{i-1}$ 转动时，这个刚体的角速度是 $\boldsymbol\omega = \dot q_i\, \mathbf z_{i-1}$（角速度沿转轴，与轴上取哪个点无关）；刚体上任意一点的线速度由刚体速度场公式给出——取轴上的点 $\mathbf p_{i-1}$ 为参考点（它不动），末端点 $\mathbf p_e$ 的速度是

$$
\mathbf v_e = \boldsymbol\omega \times (\mathbf p_e - \mathbf p_{i-1}) = \dot q_i\, \mathbf z_{i-1} \times (\mathbf p_e - \mathbf p_{i-1})
$$

除以 $\dot q_i$ 就是第 $i$ 列的上半块；下半块 $\mathbf z_{i-1}$ 是角速度贡献（角速度矢量直接相加，这是角速度叠加定理）。移动关节更简单：整段刚体平移，线速度沿轴、角速度为零。**雅可比不是求偏导求出来的，是刚体速度场拼出来的**——这也是它叫"几何"雅可比的原因。

雅可比的用处远不止速度换算：

- **静力学对偶**：末端受外力旋量 $\mathbf F$、关节出力矩 $\boldsymbol\tau$，系统准静态平衡时对任意虚位移 $\delta\mathbf q$ 虚功为零：$\boldsymbol\tau^\top \delta\mathbf q - \mathbf F^\top \delta\mathbf x = 0$。代入 $\delta\mathbf x = \mathbf J\,\delta\mathbf q$ 得 $(\boldsymbol\tau - \mathbf J^\top \mathbf F)^\top \delta\mathbf q = 0$ 对一切 $\delta\mathbf q$ 成立，故 $\boldsymbol\tau = \mathbf J^\top \mathbf F$。**速度正着映射、力反着映射**（对偶），力控和碰撞检测都靠它；
- **数值逆解**：见第 5 节；
- **奇异性分析**：见下面。
...
伪逆在奇异附近病态（对应 $1/\sigma_{min}$ 爆炸）。病根在优化目标上：伪逆只最小化 $\|\mathbf J\Delta\mathbf q - \mathbf e\|^2$，对"$\Delta\mathbf q$ 本身有多大"毫不在意。**阻尼最小二乘**（Damped Least Squares / Levenberg–Marquardt）把步长也写进代价：

$$
\Delta\mathbf q^* = \arg\min_{\Delta\mathbf q}\; \|\mathbf J\Delta\mathbf q - \mathbf e\|^2 + \lambda^2 \|\Delta\mathbf q\|^2
$$

这是无约束二次优化，令梯度为零：$2\mathbf J^\top(\mathbf J\Delta\mathbf q - \mathbf e) + 2\lambda^2 \Delta\mathbf q = \mathbf 0$，整理成正规方程

$$
\left(\mathbf J^\top \mathbf J + \lambda^2 \mathbf I\right) \Delta\mathbf q = \mathbf J^\top \mathbf e
\quad\Longrightarrow\quad
\Delta\mathbf q = \mathbf J^\top \left(\mathbf J \mathbf J^\top + \lambda^2 \mathbf I\right)^{-1} \mathbf e
$$

（第二个等号用了恒等式 $(\mathbf J^\top\mathbf J + \lambda^2\mathbf I)^{-1}\mathbf J^\top = \mathbf J^\top(\mathbf J\mathbf J^\top + \lambda^2\mathbf I)^{-1}$——两边同乘展开即证；实现上选择求逆的矩阵维度更小的那个形式，6 轴臂用 $6\times 6$ 的右式。）

它到底改了什么，用 SVD 看得最透。代入 $\mathbf J = \mathbf U \boldsymbol\Sigma \mathbf V^\top$：

$$
\Delta\mathbf q = \sum_i \frac{\sigma_i}{\sigma_i^2 + \lambda^2}\, \left(\mathbf u_i^\top \mathbf e\right) \mathbf v_i
\qquad\text{对比伪逆的}\qquad
\Delta\mathbf q = \sum_i \frac{1}{\sigma_i}\left(\mathbf u_i^\top \mathbf e\right) \mathbf v_i
$$

每个奇异方向的增益从 $\dfrac{1}{\sigma_i}$ 换成了 $\dfrac{\sigma_i}{\sigma_i^2 + \lambda^2}$：$\sigma_i \gg \lambda$ 时二者几乎相同（阻尼不干扰正常方向）；$\sigma_i \to 0$ 时增益不再爆炸而是**平滑归零**（增益峰值 $\tfrac{1}{2\lambda}$ 出现在 $\sigma_i = \lambda$ 处）——用小的跟踪误差换取解的有界性。$\lambda$ 可以按 $\sigma_{min}$ 自适应调节，只在奇异邻域内起作用。
